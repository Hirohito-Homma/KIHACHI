...
ありませんでした。語彙は**13 trait / 60表現**しかありません（`dark`/`bright`/`swung`/`straight`は2026-08-17追加）。
...
**1つ目はスキーマ。**返せるtrait名は`intent.TRAIT_WORDS`の13個だけで、polarityは±1、
...
**注意: このアダプターは実機で走らせていません。**このマシンには`ANTHROPIC_API_KEY`も
`ant` CLIもないためで、`prepare`・スキーマ・検証・CLIまではテスト済みですが、
実際のAPI呼び出しは未確認です。
...
| `intent`の語彙は比喩で書かれたブリーフをほとんど捕まえない | 「夜の高速道路を走っているような、暗くて疾走感のある」で当初trait 0件・全文`unmapped`。`dark`/`bright`追加後は`暗くて`のみ拾い、残る3節は依然`unmapped`（2026-08-17） | モデルの失敗ではなく仕様（ADR-0011）。`unmapped`を読んで語彙側を広げる。次の入口は`疾走感`（テンポではなく密度と音価の話）と、`視界が開ける`のようなセクション間の対比 |
| ジャンルはスウィングを教えてくれない | `groove.swing`を設定するジャンルは1021中`mutation_funk`（0.54）**だけ**。Jazzを含む23ファミリー全部が`None`＝0.5（ストレート）。「ジャズ」とだけ書いた曲はストレート8分で書かれます（2026-08-17） | ブリーフで明示する（`スウィング`／`シャッフル`／`跳ねる`）。2026-08-17以降、これらは`groove.swing`に届き、composerの裏拍が実際に動きます（0.607で110 BPM時20.4 ms）。ジャンル表そのものは未修正 |
