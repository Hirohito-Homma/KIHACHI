...
ありませんでした。語彙は**9 trait / 34表現**しかありません。
...
**1つ目はスキーマ。**返せるtrait名は`intent.TRAIT_WORDS`の9つだけで、polarityは±1、
...
| `intent`の語彙は比喩で書かれたブリーフを1語も捕まえない | 「夜の高速道路を走っているような、暗くて疾走感のある」でtrait 0件・全文`unmapped`。同じモデルが、語彙の語を含むブリーフからは6 traitを強度込みで正しく返す（2026-08-17） | モデルの失敗ではなく仕様（ADR-0011）。`unmapped`を読んで語彙側を広げる。9 traitに`暗い`に当たる語が無いことが最初の入口 |
