...
何から何へ移調すべきかが決まらない**のです。したがって現状で成立するのは：

- **音程を持たない素材（ドラム・パーカッション・質感）** — キーが不要。stem比でドラムが
  最大（57.9〜80.6%）で、モデルが最も安定して作る部分でもあります
- **音程を持つ素材** — 単音ピッチトラッカーが要ります。現状ありません
...
**`shortlist`の4次元は素材の選別には使えません。**duration・tempo・section_boundaries・
section_energyはいずれもアレンジの整合を測るもので、4小節のループには意味がありません。
素材用の選別指標は未着手です。
